...
test('healthy scheduled remains distinct from offline', () => {
  assert.equal(deriveVisibleStatus(state({ activity: ACTIVITY.SCHEDULED })), ACTIVITY.SCHEDULED)
})

test('offline health is visibly offline rather than stale activity', () => {
  assert.equal(
    deriveVisibleStatus(state({ health: HEALTH.OFFLINE, activity: ACTIVITY.WORKING })),
    HEALTH.OFFLINE
  )
})
